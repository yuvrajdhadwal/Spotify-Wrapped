...
def test_display_genres_single_success(mock_filter, mock_request):
    """
    Test display_genres with SpotifyWrapped data.
    """
    mock_request.GET.get.side_effect = lambda key: {"id": "1", "isDuo": "false"}.get(key)
    mock_filter.return_value.values.return_value = [{"favorite_genres": ["Genre 1", "Genre 2"]}]
    response = display_genres(mock_request)
    assert response.status_code == 200
    data = response.json()
    assert data["genres"] == "Genre 1, Genre 2"
...
@pytest.mark.django_db
@patch("spotify_data.models.DuoWrapped.objects.filter")
def test_display_songs_duo_success(mock_filter, mock_request):
    """
    Test display_songs with DuoWrapped data.
    """
    mock_request.GET.get.side_effect = lambda key: {"id": "1", "isDuo": "true"}.get(key)
    mock_filter.return_value.values.return_value = [{
        "favorite_tracks": [
            {"name": "Track 1", "artists": [{"name": "Artist 1"}], "album": {"images": [{"url": "http://example.com/track1.jpg"}]}},
            {"name": "Track 2", "artists": [{"name": "Artist 2"}], "album": {"images": [{"url": "http://example.com/track2.jpg"}]}}
        ]
    }]
    response = display_songs(mock_request)
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 2
    assert data[0]["name"] == "Track 1"
    assert data[0]["artist"] == "Artist 1"
    assert data[0]["image"] == "http://example.com/track1.jpg"


@pytest.mark.django_db
@patch("spotify_data.models.SpotifyWrapped.objects.filter")
def test_display_songs_single_success(mock_filter, mock_request):
    """
    Test display_songs with SpotifyWrapped data.
    """
    mock_request.GET.get.side_effect = lambda key: {"id": "1", "isDuo": "false"}.get(key)
    mock_filter.return_value.values.return_value = [{
        "favorite_tracks": [
...
@pytest.mark.django_db
@patch("spotify_data.models.DuoWrapped.objects.filter")
def test_display_songs_duo_no_data(mock_filter, mock_request):
    """
    Test display_songs with DuoWrapped data when no data is found.
    """
    mock_request.GET.get.side_effect = lambda key: {"id": "1", "isDuo": "true"}.get(key)
    mock_filter.return_value.values.side_effect = ObjectDoesNotExist
    response = display_songs(mock_request)
...
@pytest.mark.django_db
@patch("spotify_data.models.SpotifyWrapped.objects.filter")
def test_display_songs_single_no_data(mock_filter, mock_request):
    """
    Test display_songs with SpotifyWrapped data when no data is found.
    """
    mock_request.GET.get.side_effect = lambda key: {"id": "1", "isDuo": "false"}.get(key)
    mock_filter.return_value.values.side_effect = ObjectDoesNotExist
    response = display_songs(mock_request)
